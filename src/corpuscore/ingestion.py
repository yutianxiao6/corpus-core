...
from corpuscore.contracts.documents import ParsedDocument, SourceDescriptor
...
from corpuscore.exceptions import ConfigurationError, CorpusCoreError, UnsupportedDocumentError
from corpuscore.indexing import IndexedSource, IngestionJournal, plan_sync
from corpuscore.loaders import (
    DocxLoader,
    ExcelLoader,
    MarkdownLoader,
    PdfLoader,
...
from corpuscore.parsers import (
    CODE_EXTENSIONS,
    CodeParser,
    DocxParser,
    HtmlParser,
...
from corpuscore.ports import EmbeddingProvider, SparseEmbeddingProvider, VectorStorePort
from corpuscore.sources import (
    DiscoveryOptions,
...
class IngestionService:
    ...
    def _parse(self, source: SourceDescriptor) -> ParsedDocument:
        ...
        if suffix == ".pdf":
            ...
            try:
                ...
                if isinstance(exc, CorpusCoreError) and "OCR" in str(exc):
    ...
    def _profile_for(self, source: SourceDescriptor) -> str:
        ...
        explicit_profile = source.metadata.get("corpus_profile")
    ...
    def _failure(source_uri: str, stage: IngestionStage, exc: Exception) -> ItemFailure:
        if isinstance(exc, CorpusCoreError):
            code = exc.code
            message = str(exc)
        else:
