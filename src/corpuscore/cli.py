...
from corpuscore import __version__
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="corpus-index",
        description="Build and inspect CorpusCore document retrieval indexes.",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (CorpusCoreError, OSError, TypeError, ValueError) as exc:
        code = exc.code if isinstance(exc, CorpusCoreError) else "command_error"
...
def _configure_utf8_output(*streams: TextIO) -> None:
    """Keep Unicode document content printable on legacy Windows consoles."""
    for stream in streams:
        ...
        try:
            ...
            # In-memory and already-detached streams may not be reconfigurable.
            continue


def _init(target: Path) -> int:
    target.mkdir(parents=True, exist_ok=True)
    config_path = target / "corpus.yaml"
    if config_path.exists():
        raise ValueError(f"configuration already exists: {config_path}")
    for directory in ("data", "documents", "models"):
        (target / directory).mkdir(exist_ok=True)
    config_path.write_text(
        yaml.safe_dump(CorpusConfig().model_dump(mode="json"), allow_unicode=True, sort_keys=False),
        encoding="utf-8",
    )
...
def _preview(
    ...
    *,
    ...
    from corpuscore.config.models import RecursiveChunkProfile, SemanticChunkProfile
...
def _index(
    ...
    *,
    ...
    with RetrievalEngine.from_config(config) as engine:
...
def _activate(config: CorpusConfig, collection_name: str) -> int:
    from corpuscore.engine import RetrievalEngine
...
def _doctor(config: CorpusConfig) -> int:
...
def _backup(config: CorpusConfig, destination: Path, *, as_json: bool) -> int:
    ...
    if vector_config.mode == "local":
        ...
        from corpuscore.vectorstores import QdrantServerVectorStore
...
def _restore(config: CorpusConfig, archive: Path, *, confirmed: bool, as_json: bool) -> int:
    if not confirmed:
        raise ValueError("restore replaces the configured index; pass --yes explicitly")
    vector_config = config.vector_store
    if vector_config.mode == "local":
        from corpuscore.backup import restore_local_backup
        ...
        from corpuscore.backup import read_server_backup
