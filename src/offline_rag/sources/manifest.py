...
from offline_rag.config.loader import _UniqueKeyLoader
from offline_rag.contracts.common import JSONValue, freeze_json
...
class ManifestSourceProvider:
    ...
    def discover(self) -> Iterable[SourceDescriptor]:
        ...
        for item in self._config.documents:
            ...
            if item.profile:
                metadata["rag_profile"] = item.profile
...
def apply_sidecar(source: SourceDescriptor) -> SourceDescriptor:
    """Apply a sibling ``filename.ext.rag.yaml`` unless manifest metadata already wins."""

    path = _source_path(source.uri)
    sidecar_path = path.with_name(f"{path.name}.rag.yaml")
    if not sidecar_path.is_file():
        return source
    sidecar = _load_model(sidecar_path, SidecarConfig)
    metadata: dict[str, JSONValue] = _json_metadata(sidecar.metadata)
    if sidecar.profile:
        metadata["rag_profile"] = sidecar.profile
