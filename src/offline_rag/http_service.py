...
from offline_rag.contracts.retrieval import QueryOverrides, RetrievalResult
from offline_rag.exceptions import (
...
EngineFactory = Callable[[RagConfig], HttpEngine]
...
def create_app(
    ...
    *,
    ...
    @app.exception_handler(OfflineRagError)
    async def offline_rag_error(_request: Request, exc: OfflineRagError) -> JSONResponse:
        return JSONResponse(
...
def _default_engine_factory(config: RagConfig) -> HttpEngine:
    from offline_rag.engine import OfflineRagEngine

    return OfflineRagEngine.from_config(config)


def _engine(request: Request) -> HttpEngine:
    current = cast(HttpEngine | None, getattr(request.app.state, "engine", None))
    if current is None:
        raise OfflineResourceMissingError("retrieval engine is not ready")
    return current


def _status_for(exc: OfflineRagError) -> int:
    if isinstance(exc, ConcurrentAccessError):
        return 429
    if isinstance(
        exc,
        (OfflineResourceMissingError, VectorStoreError, IndexCompatibilityError),
    ):
        return 503
    return 400
