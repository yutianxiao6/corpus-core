...
```python
from corpuscore import RetrievalEngine, load_config
...
`RetrievalEngine` 可生成标准 `BaseRetriever`，直接用于 LangChain Runnable、Chain 或 Agent：

```python
from corpuscore import QueryOverrides
...
```python
async with RetrievalEngine.from_config(config) as engine:
...
corpus-index --config corpus.yaml query "安装方法" --profile fast --json
corpus-index --config corpus.yaml query "ERR-1042" --profile balanced \
  --organizer debug --final-k 8 --score-threshold 0.4 \
  --filter metadata.department=support --json
corpus-index --config corpus.yaml doctor
```
